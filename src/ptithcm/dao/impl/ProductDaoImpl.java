...
public class ProductDaoImpl extends connectDB implements ProductDao {
	...
	public List<Product> getProductNew() {
		List<Product> products = new ArrayList<Product>();
		String sql = "select top(4) * from product order by created desc";
		Connection conn = connectDB.getConnect(); 
	...
	@Override
	public List<Product> getBanChay() {
		List<Product> products = new ArrayList<Product>();
		String sql = "select id, catalog_id, name, price, status, description, content, discount, image_link, created from (select product_id, sum(qty) as t from ordered group by product_id) as a left join product as pd on a.product_id = pd.id order by t desc";
		Connection conn = connectDB.getConnect(); 
